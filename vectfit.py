...
def mvf(f,s,poles,tol=1e-6,iter_max=1000):
    """
    f = complex data to fit
    s = j*frequency
    poles = initial poles guess
    ...
    returns adjusted poles
    """
    new_poles = vectfit_step(f,s,poles)
    iter_i = 1
    error = 1e3
    poles_old = poles
    while error > tol and iter_i <= iter_max:
        print(f'{iter_i}, error: {error}')
        residues,d,h,poles_new = vectfit_step(f,s,poles_old)
        error = np.linalg.norm(poles_old) - np.linalg.norm(poles_new)       
        poles_old = poles_new
        if iter_i % 50 == 0:
            print(f'{iter_i}, error: {error}')
        iter_i += 1
    ...
    if iter_i > iter_max:
        ...
        print(f'Converged in {iter_i} iterations')    
    
    return residues,d,h,poles_new
