...
params= {'Rin':0.5,'V':1,'Vbase':66e3,'Rload': 200,'phi':np.pi/4*0,'SCR':2,'XR':2}
...
plt.savefig(f'{l_path}\\3phase_grid_results_tds_primitive_{t2}.pdf')
...
# df = pd.read_excel(f'{w_path}\\MLE_assesment_grid.xlsx',header=0)
df = pd.DataFrame()

# print(res)
for opt_params in [['SCR'],['XR']]:
    for scr in [0.25,0.5,1,2,3,5,7.5,10]:
        for xr in [0.25,0.5,1,2,3,5,7.5,10]:
            params= {'Rin':0.5,'V':1,'Vbase':66e3,'SCR':scr,'XR':xr}
            success = False
            data = {}
            
            key = f'{model}-' + '_'.join(opt_params) + f'-{scr}-{xr}' 
            print(key)
            ...
            df.to_excel(f'{w_path}\\MLE_assesment_grid.xlsx',header=True,index=False)
