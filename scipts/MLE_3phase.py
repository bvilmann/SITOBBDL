...
df = obtain_results(t, y[3:6,:], 'i2', 'Sim.', df=df)
df = obtain_results(t, y[6:9,:], 'v2', 'Sim.', df=df)
...
opt_params  = [f'{ABC}0{j}' for ABC in ['R','L','G','C'] for j in range(2)] + ['Rin','Rload']
...
df.to_excel(f'{w_path}\\MLE_3c_all_params_{name}.xlsx',header=True,index=True)
